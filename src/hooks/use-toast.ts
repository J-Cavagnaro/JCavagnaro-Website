...
type AddToastAction = { type: "ADD_TOAST"; toast: Toast };
type UpdateToastAction = { type: "UPDATE_TOAST"; toast: Toast };
...
function toast(props: Omit<Partial<Toast>, "id">): {
  id: string;
  dismiss: () => void;
...
function useToast() {
  ...
  React.useEffect(() => {
    listeners.push(setState as Listener);
    return () => {
      const index = listeners.indexOf(setState as Listener);
      if (index > -1) {
